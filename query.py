...
def units_with_no_exam(graph, level: int):
    print(f"Find the level {level} units that do not have an exam, and where none of their prerequisites have an exam.")
    return list(graph.query(
        f"""
        SELECT ?unit
        WHERE {{
...
def units_outside_major(graph, major_code: str, unit_code: str):
    print(f"Find the units outside of {major_code} that have a {unit_code} as a prerequisite")
    return list(graph.query(
        f"""
        SELECT DISTINCT ?unit
        WHERE {{
...
def majors_with_less_hours(graph, hours: int, contact_hour: str):
    print(f"Find the majors that have less than {hours} contact hours in total that are not {contact_hour} (summing all different types of contact hours across all units)")
    return list(graph.query(
        f"""
        SELECT ?major
        WHERE {{
...
def majors_with_less_units(graph, major_code: str, units: int):
    print(f"Which majors can I transfer to from my current completed major so that I only have to take no more than {units} more units")
    majors = []
