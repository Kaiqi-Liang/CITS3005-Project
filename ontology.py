...
with onto:
    ...
    class IsLevel(Unit >> Level, FunctionalProperty): pass
    ...
    class UnitDisjuntContains(UnitDisjunct >> Unit): pass
    ...
    class HasOutcome(Unit >> str): pass
...
with open("units.json", "r") as units:
    for unit in json.load(units).values():
        ...
        if "outcomes" in unit:
            unit_instance.HasOutcome = [outcome.strip() for outcome in unit["outcomes"]]
        ...
        for ast in unit["assessment"]:
            ...
            elif "report" in ast or "portfolio" in ast or "project" in ast or "assignment" in ast:
            ...
            elif "practical" in ast or "trip" in ast or "site" in ast or "visit" in ast or "lab" in ast:
...
with open("majors.json", "r") as majors:
    for major in json.load(majors).values():
        ...
        if "outcomes" in major:
            major_instance.HasOutcome = [outcome.strip() for outcome in major["outcomes"]]

        major_instance.HasUnit = [Unit(f'unitdetails?code={unit_code.strip()}') for unit_code in major['units']]
